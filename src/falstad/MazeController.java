...
public class MazeController implements Order { 
	 ...
	 */
	protected void switchToTitleScreen(boolean cancelOrder) {
		...
		//setState(StateGUI.STATE_TITLE_BUILDER);
		setState(StateGUI.STATE_TITLE);
		notifyViewerRedraw() ;
	}
	/**
	 * Switches to driver title screen to choose driver
	 * @param key
	 */
	protected void switchToTitleDriverScreen(int key) {
		switch (key) {
		case 'p':
			this.setBuilder(Builder.Prim);
			break;
		case 'e':
			this.setBuilder(Builder.Eller);
			break;
		default: 
			this.setBuilder(Builder.DFS);
		}
		setState(StateGUI.STATE_TITLE_DRIVER);
		notifyViewerRedraw() ;
	}
	/**
	 * Switches to driver title screen to choose skill level
	 * @param key
	 */
	protected void switchToTitleScreen(int key) {				//switch to skill level screen
		assert (state == StateGUI.STATE_TITLE_DRIVER) ;
		System.out.println("Driver key is: " + key ) ;
		switch (key) {
		case 'm':
			this.setDriver(null);
			break;
		case 'f':
			...
			break;
		case 'p':
			this.setDriver(new Pledge() );
			break;
		case 'e' :
			this.setDriver(new Explorer() );
			break;
		/*case 'w': 							// stays as Wizard;
			this.setDriver(new Wizard());
			System.out.println("wizard selected" + this.getDriver());
			break;*/
		default :
			this.setDriver(new Wizard());
	...
	protected void notifyViewerRedraw() {
		...
		while (it.hasNext())
		{
			...
			panel.whileViewerRedraw(it, state, px, py, viewdx, viewdy, walkStep, rset, angle);
		}
	 ...
	 */
	protected void notifyViewerIncrementMapScale() {
		// go through views and notify each one
		Iterator<Viewer> it = views.iterator() ;
		while (it.hasNext())
		{
	 ...
	 */
	protected void notifyViewerDecrementMapScale() {
		// go through views and notify each one
		Iterator<Viewer> it = views.iterator() ;
		while (it.hasNext())
		{
	...
	/*public boolean keyDown(int key) {
		...
		case STATE_TITLE_BUILDER:
			if (key == Constants.ESCAPE) {
				switchToTitleScreen(true) ;				//takes to builder screen
			}
			switchToTitleDriverScreen(key);
			break;
	...
	protected void setDriver(RobotDriver driver) {
		this.driver = driver ;
		if (this.driver != null) {
	...
	protected void setState(StateGUI newState) {
		...
		case STATE_TITLE_BUILDER:
			...
			assert (newState == StateGUI.STATE_TITLE_BUILDER || newState == StateGUI.STATE_TITLE_DRIVER) : msg;
			break; 
		case STATE_TITLE_DRIVER:
			assert (newState == StateGUI.STATE_TITLE || newState == StateGUI.STATE_TITLE_BUILDER) : msg ;
			break; 
		case STATE_TITLE:
			assert (newState == StateGUI.STATE_GENERATING || newState == StateGUI.STATE_TITLE_BUILDER || newState == StateGUI.STATE_TITLE) : msg ;	
			break;
		case STATE_GENERATING:
			//assert (newState == StateGUI.STATE_TITLE_BUILDER || newState == StateGUI.STATE_PLAY) : msg ;
			assert (newState == StateGUI.STATE_TITLE || newState == StateGUI.STATE_PLAY) : msg ;
			break;
		case STATE_PLAY:
			//assert (newState == StateGUI.STATE_TITLE_BUILDER || newState == StateGUI.STATE_FINISH) : msg ;
	...
	protected void toggleShowMaze() {
		showMaze = !showMaze;
	}
	protected void toggleShowSolution() {
		showSolution = !showSolution;
	}
}
