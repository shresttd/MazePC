...
public class MazeView extends DefaultViewer {
	...
	public void redraw(MazePanel panel, StateGUI state, int px, int py, int view_dx,
		...
		case STATE_TITLE_BUILDER:
			redrawTitleBuilder(panel.getBufferGraphics());
			break;
		case STATE_TITLE_DRIVER:
			redrawTitleDriver(panel.getBufferGraphics());
			break;
		case STATE_TITLE:
		...
		case STATE_FINISH:
			if (controller.getRobot().hasStopped()) {
				redrawFinishLost(panel.getBufferGraphics());
			}
			else{
	...
	void redrawTitle(Graphics gc) {
		...
		//centerString(gc, fm, "", 200);
		//centerString(gc, fm, "To start, select a skill level", 200);
		centerString(gc, fm, "Select a builder, a driver and a skill level,", 200);
		centerString(gc, fm, "and press START to start the game!", 225);
		centerString(gc, fm, "After the game starts", 285);
		centerString(gc, fm, "Press ENTER to start the driver!", 310);	
		centerString(gc, fm, "Version 2.0", 350);
	}
	void redrawTitleOriginal(Graphics gc) {
		// produce white background
		gc.setColor(Color.white);
		gc.fillRect(0, 0, Constants.VIEW_WIDTH, Constants.VIEW_HEIGHT);
		// write the title 
		...
		gc.setColor(Color.black);
		centerString(gc, fm, "To start, select a skill level", 200);
		centerString(gc, fm, "Press a number from 0 to 9,", 225);
		centerString(gc, fm, "or a letter from A to F", 250);
		centerString(gc, fm, "After the game starts", 285);
		if (controller.getDriver() != null) {
			centerString(gc, fm, "Press ENTER to start the driver!", 310);
		}
		else {
			centerString(gc, fm, "Use the ARROW KEYS to navigate the maze!", 310) ;
		}
		centerString(gc, fm, "Version 2.0", 350);
	}
	...
	void redrawFinish(Graphics gc) {
		...
		if (controller.getDriver() != null) {
		    centerString(gc, fm, "You took " + controller.getDriver().getPathLength() + " steps!", 210) ;
		    centerString(gc, fm, "You consumed " + controller.getDriver().getEnergyConsumption() + " units of energy!", 235) ;
		}
		else {
	        centerString(gc, fm, "You took " + controller.getManualDriver().getPathLength() + " steps!", 210) ;
			centerString(gc, fm, "You consumed " + controller.getManualDriver().getEnergyConsumption() + " units of energy!", 235) ;
		}		
		// write the instructions
		gc.setColor(Color.white);
	...
	void redrawFinishLost(Graphics gc) {
		// produce blue background
		gc.setColor(Color.blue);
		gc.fillRect(0, 0, Constants.VIEW_WIDTH, Constants.VIEW_HEIGHT);
		// write the title 
		...
		gc.setFont(smallBannerFont);
		fm = gc.getFontMetrics();
		centerString(gc, fm, "You ran out of battery", 160);
		gc.setColor(Color.orange);
