...
public class FirstPersonDrawer extends DefaultViewer {
	...
	private final static Logger logFirstPerson = Logger.getLogger(FirstPersonDrawer.class.getName());
	...
	public void redraw(MazePanel panel, StateGUI state, int px, int py, int view_dx, int view_dy, int walk_step, int view_offset, RangeSet rset, int ang) {	
		...
		/*this.gc = (Graphics2D) panel.getBufferGraphics() ; 
	...
	private boolean boundingBoxIsVisible(BSPNode b) {
		...
		// constraint: x1 <= x2
		int[] p = {x1, x2} ; //Point p = new Point(x1, x2);
	...
	private void drawSegment(Seg seg, MazePanel panel) {
		...
		panel.setColorSeg(seg.getColor());
		...
		while (x1i <= x2) {
			...
			int[] p = {x1i, x2} ; //Point p = new Point(x1i, x2);
			if (!rset.intersect(p))
				break;
			x1i = p[0]; //x1i = p.x;
			int x2i = p[1] ; //int x2i = p.y;
	...
	private void dbg(String str) {
		...
		logFirstPerson.info("FirstPersonDrawer:"+ str);
	}
